...
export async function GET() {
  try {
    ...
    // Fetch 40 pages in parallel to cover ~1 month of articles
    const pages = await Promise.allSettled(
      Array.from({ length: 40 }, (_, i) => fetchPage(i + 1))
    );
    ...
    for (const result of pages) {
      ...
      for (const { title, url, desc } of result.value) {
        ...
        // Drop articles older than 30 days (keep undated ones)
    ...
    return NextResponse.json({ articles: articles.slice(0, 500) });
  } catch (err) {
